...
File Description: Latest WooCommerce Product Class
...
if ( class_exists('Plethora_Widget') && class_exists('Plethora_Module_Woocommerce') && class_exists('woocommerce') && !class_exists('Plethora_Widget_LatestProducts') ) {
 
     /**
      * @package Plethora Base
      */
     add_action('widgets_init', create_function( '','return register_widget("Plethora_Widget_LatestProducts");') );

     class Plethora_Widget_LatestProducts extends Plethora_Widget  {

        public $widget_class = 'Plethora_Widget_LatestProducts'; 
 

          function __construct() {

               $widget_ops = array(
                    'classname'   => 'latestwooproducts',
                    'description' => 'Latest WooCommerce Products'
               );

               $control_ops = array( 'id_base' => 'latestwooproducts-widget' );
               parent::__construct( 
                    'latestwooproducts-widget',
                    '> PL | Latest Products',
                    $widget_ops,
                    $control_ops
               );
          ...
          function widget( $args, $instance ) {
               ...
               $cache = wp_cache_get('widget_latestwooproducts_posts', 'widget');
               ...
               $title    = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Latest Products', 'cleanstart' );
               ...
               if ( !empty($category)) { 
                  $tax_query_args = array( array( 'taxonomy' => 'product_cat', 'field' => 'slug', 'terms' => $category ));
                  $ln_query_args['tax_query'] = $tax_query_args;
               }
               ...
               <?php while ( $ln_query->have_posts() ) : $ln_query->the_post(); ?>
                    ...
                    <?php
                        ...
                        if ( has_post_thumbnail( get_the_ID() )) {
                            $latestwooproducts_featured_image  = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ) );
                            echo '<a href="' . get_permalink() . '" class="media-photo" style="background-image:url(' . esc_url( $latestwooproducts_featured_image[0] ) . ')"></a>'; 
                            $xtra_class = "";
                      } else {
               ...
               wp_cache_set('widget_latestwooproducts_posts', $cache, 'widget');
          ...
          function update( $new_instance, $old_instance ) {
               ...
               if ( isset($alloptions['widget_latestwooproducts_entries']) )
                    delete_option('widget_latestwooproducts_entries');
          ...
          function flush_widget_cache() {
               ...
               wp_cache_delete( 'widget_latestwooproducts_posts', 'widget' );
           ...
           */
           public static function get_feature_options() {
              ...
              $feature_options = array ( 
                  'switchable'         => true,
                  'options_title'      => 'Latest WooCommerce Products widget',
                  'options_subtitle'   => 'Activate/deactivate "Latest Products" widget',
